import { ArrowLeft, Eye, EyeOff, ShieldCheck, Wifi } from 'lucide-react'
    ...
    <main className="min-h-screen bg-ink px-5 py-7 sm:px-8 sm:py-10">
      <div className="mx-auto max-w-5xl">
        ...
        {isLoading ? <div className="mt-8 h-72 animate-pulse rounded-[30px] bg-white/[0.04]" /> : <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <aside className="surface p-6"><div className="flex items-center gap-2 text-brand"><ShieldCheck size={19} /><p className="text-sm font-bold">Resumo do cartão</p></div><div className="mt-7 space-y-5"><div><p className="text-sm text-zinc-500">Fatura atual</p><p className="mt-1 text-2xl font-extrabold tracking-[-0.04em] text-white">{formatCurrency(pendingInvoice || Number(card?.currentSpending ?? 0))}</p></div><div className="h-px bg-white/[0.07]" /><div><p className="text-sm text-zinc-500">Limite total</p><p className="mt-1 text-lg font-bold text-zinc-200">{formatCurrency(Number(card?.creditLimit ?? 0))}</p></div></div><button onClick={() => setSearchParams({ acao: 'fatura' })} className="focus-ring mt-8 w-full rounded-2xl bg-brand px-4 py-3 text-sm font-bold text-black shadow-brand hover:bg-[#ffe633]">Pagar fatura</button><button onClick={() => setSearchParams({ acao: 'limite' })} className="focus-ring mt-3 w-full rounded-2xl border border-white/[0.1] px-4 py-3 text-sm font-semibold text-zinc-300 hover:bg-white/[0.05]">Solicitar limite</button></aside>
        </div>}
        {!isLoading && <section className="surface mt-6 p-6"><h2 className="text-xl font-bold text-white">Faturas e compras</h2><div className="mt-5 space-y-4">{invoices.length ? invoices.map((invoice) => <article key={invoice.id} className="rounded-2xl border border-white/[0.07] p-4"><div className="flex justify-between"><div><p className="font-semibold text-zinc-200">Fatura #{invoice.id}</p><p className="mt-1 text-xs text-zinc-500">{new Date(invoice.createdAt).toLocaleDateString('pt-BR')}</p></div><div className="text-right"><p className="font-bold text-brand">{formatCurrency(Number(invoice.totalAmount))}</p><p className="mt-1 text-xs text-zinc-500">{invoice.status}</p></div></div><div className="mt-4 divide-y divide-white/[0.06]">{invoice.purchases?.map((purchase) => <div key={purchase.id} className="flex justify-between py-2 text-sm"><span className="text-zinc-400">{purchase.description}</span><span className="font-semibold text-zinc-200">{formatCurrency(Number(purchase.amount))}</span></div>)}</div></article>) : <p className="text-sm text-zinc-600">Ainda não há faturas ou compras.</p>}</div></section>}
        {action && !isLoading && <section className="surface mt-6 p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-semibold text-brand">{action === 'limite' ? 'Mais limite' : 'Pagamento de fatura'}</p><h2 className="mt-1 text-xl font-bold text-white">{action === 'limite' ? 'Solicite uma nova análise' : 'Quite sua fatura atual'}</h2></div><button onClick={() => setSearchParams({})} className="text-sm font-semibold text-zinc-500 hover:text-white">Fechar</button></div>{action === 'limite' ? <form onSubmit={handleLimitRequest} className="mt-6 flex max-w-lg flex-col gap-3 sm:flex-row"><input required inputMode="decimal" value={requestedLimit} onChange={(event) => setRequestedLimit(event.target.value)} placeholder="Limite desejado, ex.: 1500,00" className="input" /><button disabled={isSubmitting} className="focus-ring rounded-2xl bg-brand px-5 py-3 text-sm font-bold text-black hover:bg-[#ffe633] disabled:opacity-60">{isSubmitting ? 'Enviando...' : 'Solicitar'}</button></form> : <form onSubmit={handleInvoicePayment} className="mt-6 max-w-lg space-y-3"><p className="text-sm text-zinc-500">Pagamento da fatura pendente de <span className="font-bold text-zinc-200">{formatCurrency(Number(pendingInvoices[0]?.totalAmount ?? 0))}</span>, usando a conta {accounts[0]?.accountNumber ?? 'indisponível'}.</p><input required type="password" value={paymentPassword} onChange={(event) => setPaymentPassword(event.target.value)} placeholder="Senha de transação" className="input" /><button disabled={isSubmitting || pendingInvoices.length === 0} className="focus-ring rounded-2xl bg-brand px-5 py-3 text-sm font-bold text-black hover:bg-[#ffe633] disabled:opacity-60">{isSubmitting ? 'Pagando...' : 'Confirmar pagamento'}</button></form>}{actionError && <p role="alert" className="mt-4 rounded-xl border border-red-500/20 bg-red-500/10 px-3 py-2 text-sm text-red-300">{actionError}</p>}{actionSuccess && <p role="status" className="mt-4 rounded-xl border border-brand/20 bg-brand/10 px-3 py-2 text-sm text-brand">{actionSuccess}</p>}</section>}
      </div>
    </main>
  )
}
